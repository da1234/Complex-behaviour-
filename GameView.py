...
#Changing between SET or RANDOM Initial Conditions
RANDOM = False
...
if SINGLE:
    ...
    all_colours = [no_of_reds, no_of_greens, no_of_blues, no_of_oranges, no_of_yellows, no_of_purples, no_of_pinks, no_of_browns, no_of_cyans]
...
while not done:
    ...
    stability = model.stability_check(all_colours)
    if stability:
        done=True
...
if SINGLE:
    ...
    plt.plot(clock_ticks, no_of_reds, 'r', label="Strat: 1")
    plt.plot(clock_ticks, no_of_greens, 'g', label="Strat: 2")
    plt.plot(clock_ticks, no_of_blues, 'b', label="Strat: 3")
    if RANDOM:  
        if PERIODIC:
    ...
    if RANDOM:  
        ...
        if PERIODIC:
            plt.title("Population of Each Strategy Against Time (NON-RANDOM, PERIODIC)")
        else:
            plt.title("Population of Each Strategy Against Time (NON-RANDOM, NON-PERIODIC)")
